...
BEGIN_MESSAGE_MAP(CMyFractalView, CView)
	...
	ON_WM_LBUTTONDOWN()
	ON_WM_RBUTTONUP()
    // Liens entre les IDs des boutons et vos fonctions C++
    ON_COMMAND(ID_DRAW_MANDELBROT, &CMyFractalView::OnDrawMandelbrot)
    ON_COMMAND(ID_DRAW_JULIA, &CMyFractalView::OnDrawJulia)
    // Pour afficher visuellement quel bouton est enfoncé
    ON_UPDATE_COMMAND_UI(ID_DRAW_MANDELBROT, &CMyFractalView::OnUpdateDrawMandelbrot)
    ON_UPDATE_COMMAND_UI(ID_DRAW_JULIA, &CMyFractalView::OnUpdateDrawJulia)
END_MESSAGE_MAP()
...
CMyFractalView::CMyFractalView() noexcept
{
    ...
    m_bDrawMandelbrot = true; // Par défaut, on dessine Mandelbrot au démarrage
	m_juliaC = std::complex<double> (-0.7, 0.27015); // Paramètre c pour l'ensemble de Julia
...
void CMyFractalView::OnDraw(CDC* pDC)
{
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            ...
            // Au lieu de: bool drawMandelbrot = true;
            // Utilisez directement la variable de la classe :
            std::complex<double> c = m_bDrawMandelbrot ? z : m_juliaC;
...
void CMyFractalView::OnLButtonDown(UINT /* nFlags */, CPoint point)
{
	...
	m_bDrawMandelbrot = true;   // Passe en mode Mandelbrot
...
void CMyFractalView::OnRButtonUp(UINT /* nFlags */, CPoint point)
{
    ...
    // m_juliaC = std::complex<double>(-0.7 + (point.x / 800.0) * 1.4, -0.7 + (point.y / 600.0) * 1.4); // Ajustez les valeurs selon vos besoins
}
...
void CMyFractalView::OnDrawMandelbrot()
{
    m_bDrawMandelbrot = true;
    Invalidate(); // Force l'appel à OnDraw
}

void CMyFractalView::OnDrawJulia()
{
    m_bDrawMandelbrot = false;
    Invalidate(); // Force l'appel à OnDraw
}

// Optionnel : Ces fonctions permettent de laisser le bouton "enfoncé" graphiquement
void CMyFractalView::OnUpdateDrawMandelbrot(CCmdUI* pCmdUI)
{
    pCmdUI->SetCheck(m_bDrawMandelbrot ? 1 : 0);
}

void CMyFractalView::OnUpdateDrawJulia(CCmdUI* pCmdUI)
{
    pCmdUI->SetCheck(!m_bDrawMandelbrot ? 1 : 0);
}
